...
from .utils.point_cloud import load_point_cloud, print_point_cloud_info, plot_point_cloud
	...
	def post(self, request):
		try:
			...
			alpha_mesh = o3d.geometry.TriangleMesh.create_from_point_cloud_alpha_shape(
				filtered_cloud, alpha=1.0
			)
			alpha_mesh.compute_vertex_normals()
			o3d.visualization.draw_geometries(
				[alpha_mesh], window_name="Malla - Umbral"
			)
	...
	def get(self, request):
		...
		if file_path:
			try:
				...
				# Visualización
				plot_point_cloud(file_path, point_cloud)
			...
			try:
				...
				for key, value in routes.items():
					...
					print_point_cloud_info(point_cloud)
					plot_point_cloud(value, point_cloud)
				return Response(
